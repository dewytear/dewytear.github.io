...
4. `python3 tools/build_index.py && python3 tools/build_dates.py`로 data/ 생성 확인.
5. `python3 tools/validate_all.py` — ERROR 0 확인.
6. CI: `.github/workflows/validate.yml`에 validate_all(push·PR) + check_worklog·check_cachebuster(PR) 스텝을 건다(레퍼런스 위키의 워크플로 참조).
7. 프로젝트 CLAUDE.md에 운영 규칙을 적는다 — 본문 추가 규칙(문서→list→doc-entries→인덱스→Work Log→검증), 빅 프레임 로그 규칙, 캐시버스터 규칙. 레퍼런스: https://github.com/dewytear/dewytear.github.io 의 CLAUDE.md.
