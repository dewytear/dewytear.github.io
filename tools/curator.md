...
- **없으면** 새로 만든다:
  1. 파일 `wl-<YYYYMMDD>-curate` 생성(다른 wl 문서 형식을 따름 — `<h2>YYYY-MM-DD · AI 지식 큐레이터</h2>` + `<p>` 요약 + `<ul><li>` 항목). 첫 항목으로 위 큐레이터 활동 줄을 넣는다.
...
**절대 하지 않을 것:** 자동 머지, `build_index.py` 로직 변경(예외: 새 클러스터 등록 시 `CLUSTER_LABELS` 항목 추가만 허용), `knowledge-index.json` 직접 편집, (인덱스·지도·오늘 Work Log·`list` 외) 관계없는 파일 수정.
