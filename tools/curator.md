...
이 문서는 **예약 AI 큐레이터**가 따르는 절차입니다. 매일 1회 새 세션에서 자동 실행되어, 위키 문서의 변경을 감지해 `knowledge-index.json`을 갱신하고 **변경이 있을 때만** 검토용 PR을 올립니다. **자동 머지는 하지 않습니다** (사람이 검토 후 머지).
...
### 5. PR 제안 (변경이 있을 때만)
```
git add tools/doc-entries.json knowledge-index.json ai-map
...
**절대 하지 않을 것:** 자동 머지, `build_index.py` 로직 변경, `knowledge-index.json` 직접 편집, 대상 외 파일 수정.
